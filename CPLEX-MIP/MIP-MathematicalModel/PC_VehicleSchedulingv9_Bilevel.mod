...
//   thisOplModel.printSolution();
   if (cplex.solve()) {
     var ofile = new IloOplOutputFile("Y:/Sentosa/Optimization_Algos/Het10TripsEVeh1CS_Idle.txt");
...
// int zeta[i in Sprime, j in Sprime] = 
 ...
 minimize sum(k in K) U[k] 
 	+ sum(k in K, c in C, i in S, j in S diff{i}, r in R) (phi[i,j,r] * (alpha * zeta_charged[i,j,r]) + (1-phi[i,j,r])*(alpha * zeta[i,j]));//zeta[i,j];//zeta[i, j, r-1];
