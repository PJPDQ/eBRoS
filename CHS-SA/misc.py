...
def feasible_recharge(data, deadheads, recharge, terminals):
    ...
    recharging_costs = (
        lambda pair: (connected_trips(pair)>=((deadheads[bus_data.loc[pair[0], "arr_term"]][recharge[pair[2]]['name']])))#or (recharge[pair[2]]['dep_term'] == bus_data.loc[pair[1], "dep_term"])
    )
    ### duration (delta)
    duration_trips = (
...
#             print(all_nodes.iloc[i, 1])
            if all_nodes.iloc[i, 1] == 'cs': ## time difference between the end of charging to the next departure
                ...
                total_waiting += all_nodes.iloc[j, 3] - all_nodes.iloc[h, 4]#recharge_arc[(h,j,i)]['duration']
...
def visualizeSolution(solution_spaces, title, all_sch, recharge_arc):
    numTrips = all_sch.loc[all_sch.type == "trip"].shape[0]
    numCS = all_sch.loc[all_sch.type == "cs"].shape[0]
    numBuses = []
    gaps = []
    ...
    # fig = ax.get_figure()
    soln_df = pd.DataFrame({'solution': solution_spaces, 'numBuses': numBuses, 'Gaps': gaps})
    ax = soln_df[['numBuses', 'Gaps']].plot(x='numBuses', y='Gaps', style='*')
    # Set integer x-axis ticks only
    ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    ax.set_xlabel("numBuses")
    ax.set_ylabel("Total Gap")
    soln_df['nBuses'] = soln_df['numBuses']
    df_least = soln_df.groupby(['nBuses'])[['numBuses', 'Gaps']].min().sort_values(by=['numBuses'], ascending=False)
    df_least['minGaps'] = df_least["Gaps"][::-1].cummin()[::-1]
    df_least.plot(x='numBuses', y='minGaps', style="-", ax=ax)
    # df_least = soln_df.groupby(['nBuses'])[['numBuses', 'Gaps']].min().sort_values(by=['numBuses'], ascending=False)
    
    # .plot(x='numBuses', y='Gaps', xlabel="numBuses", ylabel="Gaps", legend=False, ax=ax)
    fig = ax.get_figure()
    fig.savefig(f"{title}-ParetoFrontier")

    
    norm_gaps = [custom_minmax_scaler(x, min(gaps), max(gaps)) for x in gaps]
    norm_nbuses = [custom_minmax_scaler(x, min(numBuses), max(numBuses)) for x in numBuses]
    test = pd.DataFrame({"NumBuses[#]": norm_nbuses, "IdleTime(Gap)[sec]": norm_gaps})
    ax = test.sort_values(by=['NumBuses[#]']).plot(x='NumBuses[#]', y='IdleTime(Gap)[sec]', xlabel="Norm_numBuses", ylabel="Norm_IdleTime(Gap)[sec]", legend=False, title=f"{numTrips}Trips{numCS}CS_ParetoFrontier", style="*")
    test['nBuses'] = test['NumBuses[#]']
    df_least = test.groupby(['nBuses'])[['NumBuses[#]', 'IdleTime(Gap)[sec]']].min().sort_values(by=['NumBuses[#]'], ascending=False)
    df_least['minGaps'] = df_least["IdleTime(Gap)[sec]"][::-1].cummin()[::-1]
    df_least.plot(x='NumBuses[#]', y='minGaps', style="-", ax=ax)
    # Set integer x-axis ticks only
    #ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    fig2 = ax.get_figure()
    fig2.savefig(f"{title}-Norm-ParetoFrontier")
    return soln_df
...
#                 dur = xf-xs
#                 ax.plot([xs, xf], [mdx]*2, c=colors[jdx%len(trips)], **bar_style)
#                 if dur < 100:
    ...
    for bus_id, schedule in enumerate(schedules, start=1):
        for s in schedule:
            ...
            if s in cs_ids:
                ...
                bus['dep_time'] = (all_sch.loc[all_sch.ID == schedule[idx-1], 'arr_time'].iloc[0])
...
#                     print(f"dur = {xs}.. {xf}")
#                     ax.plot([xs, xf], [mdx]*2, c="yellow", **bar_style)
#                     ax.text((xs + xf)/2, mdx, j, **small_text_style)
...
def visualizeResult(schedules, all_sch, title, cs_deadheads):
    cs_ids = all_sch.loc[all_sch.type == "cs", 'ID'].to_list()
    trips_ids = all_sch.loc[all_sch.type == "trip", 'ID'].to_list()
    depot = all_sch.loc[all_sch.type == "depot", 'ID'].to_list()
    res_dict = []
    for bus_id, schedule in enumerate(schedules, start=1):
        for s in schedule:
            ...
            if s in cs_ids:
                idx = schedule.index(s)
                bus['trip_id'] = all_sch.loc[all_sch.ID == s]['name'].iloc[0]
                bus['dep_time'] = (all_sch.loc[all_sch.ID == schedule[idx-1], 'arr_time'].iloc[0]) #+ (cs_deadheads[all_sch.loc[all_sch.ID == schedule[idx-1], 'arr_term'].iloc[0]][all_sch.loc[all_sch.ID == s, 'name'].iloc[0]]//2)
                bus['arr_time'] = bus['dep_time'] + all_sch.loc[all_sch.ID == s, 'duration'].iloc[0]
                bus['duration'] = all_sch.loc[all_sch.ID == s, 'duration'].iloc[0] + CHARGING_TIME
                bus['dep_terminal'] = all_sch.loc[all_sch.ID == schedule[idx-1], 'arr_term'].iloc[0]
                bus['arr_terminal'] = all_sch.loc[all_sch.ID == schedule[idx-1], 'arr_term'].iloc[0]
            elif s in depot:
                continue
