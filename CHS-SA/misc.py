...
def feasible_recharge(data, gamma, recharge):
    bus_data = data.copy(deep=True)
    bus_data = bus_data.reset_index()
    bus_data.rename(columns={'index': 'trip_id'}, inplace=True)
    bus_data.index = np.arange(0, len(data))
...
#             print(all_nodes.iloc[i, 1])
            if all_nodes.iloc[i, 1] == 'cs': ## time difference between the end of charging to the next departure
                h = schedule[idx-1]
                charge_time = all_nodes.iloc[h, 4] + recharge_arc[(h,j,i)]['duration'] # end of charging + deadhead to the next schedule (assumed to be equidistant)
                total_waiting += (all_nodes.iloc[j, 3] - charge_time)
...
def visualizeSolution(solution_spaces, title, all_schs, recharge_arcs):
    numBuses = []
    gaps = []
    for space in solution_spaces:
        numBuses.append(len(space))
        gap = get_total_gap(space, all_schs, recharge_arcs)
        gaps.append(gap)
...
#                 xf_str = schedule.loc[(j,m), 'arr_str'].iloc[0]
                dur = xf-xs
                print(dur)
                ax[0].plot([xs, xf], [jdx]*2, c=colors[mdx%10], **bar_style)
                ax[1].plot([xs, xf], [mdx]*2, c=colors[jdx%10], **bar_style)
                if dur < 120:
